...
production data the claim assumed. Three PRs measured zero at merge for
exactly that reason: #2360's classification is observability (the 5,208
reads/hr are *classified*, not eliminated), #2399's four configs all shipped
with `polling.intervals` commented out so no gate binds, and #2417's cache
validator never fired because zero of the 462 cached bodies have its shape.
...
deep analysis. The reviewer-side check lives in the `aiur-run` skill
