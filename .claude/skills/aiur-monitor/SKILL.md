...
`iarc` is an Executor alias for `aiur` in this repo. Treat `/iarc status` as this
...
anything actionable) and `--full` when the Executor asks for the whole board or
...
Post the `aiurdev watch` output to the Executor as-is (a fenced block is fine).
...
- **`⚠️ Needs you`** — when the `ACTIONABLE` section is non-empty, add ONE line
  below the board, `⚠️ Needs you: #<id> (<why>)`, naming what the Executor must
...
- **Daemon down.** If `aiurdev watch` reports the orchestrator is not running
  ...
  `🔴 daemon down` line instead of "no active agents", and tell the Executor the
...
Executor wants that state rolled up into **two markdown status tables plus Decisions** so a glance
...
Executor sees the complete arc. Columns: `Ticket | Description | Phase | Status`. Status markers:
...
(e.g. `🔴 in-progress — BLOCKS ALL MERGES`) so the Executor can see at a glance what is gating the
rest.
...
unresolved escalation until it is answered; the durable ledger retains older rows so an Executor
...
via `aiur-run`, or one is running in this repo) you **MUST** post the Executor a fresh board
(run `aiurdev watch` → post it) **every `<the Executor’s chosen interval>`** (established once by
...
interval), **automatically**, until the run reaches a terminal state or the Executor says stop —
you do not get to opt out of "watching"; a live run obligates the cadence. 5 minutes is the
recommended default; the Executor picks the value once and you never re-ask it.
...
- The Executor should **NEVER** have to ask for the next update. The cadence is automatic;
...
- **Terminal / stop only.** Keep looping until the Executor explicitly stops it OR the run has
...
- A single one-shot snapshot (no cadence) is allowed only when the Executor explicitly asks for
...
`aiurdev watch`'s `ACTIONABLE` section is the **pull** path — actionable alerts reach the Executor
...
line is one Monitor event — post it in chat so the Executor gets the "why" the instant they hear
the chime:
...
the recurring cadence timer; never put the Executor behind a foreground `sleep`. A
`needs_attention:true` Monitor event re-invokes the Executor immediately, interrupts whatever
...
For any **NEW** `needs_attention:true` alert, also relay it via the active Executor notification
...
2. Notify every active surface; a failed surface is recorded beside that Decisions entry and is
   ...
   - **Claude Executor:** use Claude's native **PushNotification**.
   - **Codex Executor:** use Codex's device notification when available; otherwise use the shared
   ...
   - **Remote-control mode:** additionally invoke the existing RC notification path with the same
     formatted alert whenever the Executor’s RC session is active. RC is additive to the backend
...
### Executor decision policy: medium autonomy

The Executor applies the same boundary whether a question arrives from the wake watcher or is
...
At launch, `aiur-run` records the active backend from the Executor session (not `agent.kind`,
