...
test('ticket context keeps semantic content, touch targets, and origin focus accessible at responsive sizes', async ({ browser }) => {
  ...
  for (const { width, isMobile, forcedColors } of [
    ...
    try {
      ...
      // Focus starts on the heading and is trapped in the dialog. Tab walks the
      // header dismiss control, then the CTA row in reading order.
      await page.keyboard.press('Tab')
      await expect(close).toBeFocused()
      await page.keyboard.press('Tab')
      await expect(dialog.getByRole('link', { name: 'Open in GitHub' })).toBeFocused()
      await page.keyboard.press('Tab')
      await expect(dialog.getByRole('link', { name: 'Read chat' })).toBeFocused()
      await page.keyboard.press('Tab')
