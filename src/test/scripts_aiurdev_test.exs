defmodule ScriptsAiurdevTest do
  ...
  defp fake_repo(root \\ Aiur.TestSupport.tmp_root!("aiurdev-shim")) do
  ...
  defp fake_agent_repo(issue) do
    root =
      Path.join([
        ...
        "repo",
        to_string(issue),
        Aiur.TestSupport.tmp_root!("aiurdev-shim")
      ])
  ...
  defp fake_mise do
    path = Aiur.TestSupport.tmp_root!("aiurdev-mise")
  ...
  defp global_shim(target) do
    bin = Aiur.TestSupport.tmp_root!("aiurdev-bin")
  ...
  test "the rebuild command the shim emits actually runs, from a path with a space" do
    ...
    shim_dir = Aiur.TestSupport.tmp_root!("aiur dev shim")
  ...
  test "agent workspace non-test detection falls back to PWD and roots sandbox there" do
    ...
    pwd = Path.join([System.tmp_dir!(), "aiur-workspaces", "repo", Aiur.TestSupport.tmp_root!("482")])
    home = sandbox_home()
    mise = fake_mise()
  ...
  test "agent workspace --test detection falls back to PWD and blocks before sandboxing" do
    ...
    pwd = Path.join([System.tmp_dir!(), "aiur-workspaces", "repo", Aiur.TestSupport.tmp_root!("483")])
    home = sandbox_home()
    mise = fake_mise()
  ...
  describe "checkout targeting" do
    ...
    test "an agent workspace clone is refused a run against the operator's checkout" do
      ...
      workspace =
        Path.join([
          ...
          "aiur-workspaces",
          "repo",
          Aiur.TestSupport.tmp_root!("9001")
        ])
    ...
    test "github-cost from another checkout reads the daemon instead of being refused" do
      # `github-cost` reads the meter the daemon already keeps and builds
      # nothing, so it belongs on the read-only side of the divergence guard.
      # Listing it in one command table and not the other would refuse a
      # read-only report from any checkout but the shim's own.
      {checkout_a, shim_a} = fake_checkout()
      {checkout_b, _shim_b} = fake_checkout()
      link = global_shim(shim_a)
    ...
    test "a cwd inside a partial tree is not mistaken for a checkout" do
      ...
      partial = Aiur.TestSupport.tmp_root!("aiurdev-partial")
  ...
  describe "release provenance" do
    ...
    test "a repo root nested inside another repository is not stamped with that repo's commit" do
      ...
      outer = Aiur.TestSupport.tmp_root!("aiurdev-outer")
      File.mkdir_p!(outer)
