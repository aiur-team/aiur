defmodule AiurWeb.DashboardCredentialSupportTest do
  ...
  test "isolates and restores the complete credential boundary without changing the default state" do
    ...
    try do
      ...
      # Entry never rewrites the suite's default credential state; it only
      # clears any cached configuration generation inherited from an earlier
      # module so this module cannot issue tokens against a stale pair.
      ...
      # Simulate the module mutating the boundary in its own tests.
      ...
      # Exit restores every global the module saw, and clears the cached
      # generation so the next writer does not inherit it.
      assert System.get_env("AIUR_DASHBOARD_USERNAME") == "before-user"
      assert System.get_env("AIUR_DASHBOARD_PASSWORD") == "before-password"
      assert Application.fetch_env!(:aiur, Endpoint) == before_config
      assert Endpoint.config(:dashboard_auth_required)
