defmodule AiurWeb.BuildOrder.PlanningSourceTest do
  ...
  setup do
    path = Path.join(System.tmp_dir!(), "planning-source-test-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    workspace_directory = Path.join(System.tmp_dir!(), "planning-source-workspace-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "excludes configured packs for another repository and reports searched directories" do
    directory = Path.join(System.tmp_dir!(), "planning-source-scope-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "loads a materialized canonical pack from the repository discovery directory" do
    directory = Path.join(System.tmp_dir!(), "planning-source-published-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "hydrates canonical ticket fields from membership without tracker reads" do
    path = Path.join(System.tmp_dir!(), "planning-source-canonical-#{System.pid()}-#{System.unique_integer([:positive])}.json")
  ...
  test "does not count cancelled members as completed progress" do
    path = Path.join(System.tmp_dir!(), "planning-source-cancelled-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    File.write!(path, @canonical_pack)
    Application.put_env(:aiur, :build_order_planning_pack, path)
  ...
  test "uses status.json for canonical members when no live membership exists" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "tracker completion outranks active current-run membership" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-completed-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "tracker reopen outranks terminal current-run membership" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-open-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "marks a retained status projection stale when PackStatus is unavailable" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-stale-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "marks missing status unavailable when PackStatus has no projection" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-unavailable-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "downgrades healthy PackStatus when a promoted member lacks a projection" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-incomplete-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "preserves PackStatus budget exhaustion through an incomplete projection" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-budget-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "ignores a malformed status members shape" do
    directory = Path.join(System.tmp_dir!(), "planning-source-status-malformed-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "membership recovery does not regress the PackStatus-backed generation" do
    path = Path.join(System.tmp_dir!(), "planning-source-generation-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    File.write!(path, @mixed_pack)
    Application.put_env(:aiur, :build_order_planning_pack, path)
  ...
  test "renders created members live and uncreated members as planned from one canonical pack" do
    directory = Path.join(System.tmp_dir!(), "planning-source-mixed-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "rejects a draft document outside its pack ticket directory" do
    directory = Path.join(System.tmp_dir!(), "planning-source-document-boundary-#{System.pid()}-#{System.unique_integer([:positive])}")
    path = Path.join(directory, "build-order.json")
  ...
  test "uses live labels for created tickets and pack labels for drafts" do
    path = Path.join(System.tmp_dir!(), "planning-source-live-labels-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    File.write!(path, @mixed_pack)
    Application.put_env(:aiur, :build_order_planning_pack, path)
  ...
  test "rejects members without canonical ticket or document fields" do
    path = Path.join(System.tmp_dir!(), "planning-source-invalid-schema-#{System.pid()}-#{System.unique_integer([:positive])}.json")
  ...
  test "discovers canonical packs from the runtime build-order directory" do
    directory = Path.join(System.tmp_dir!(), "planning-source-discovery-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "does not reconcile discovery packs without explicit build order IDs" do
    first = Path.join(System.tmp_dir!(), "planning-source-missing-id-first-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    second = Path.join(System.tmp_dir!(), "planning-source-missing-id-second-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    repository = Config.repo()
  ...
  test "assigns distinct deterministic catalog icons when packs omit one" do
    first = Path.join(System.tmp_dir!(), "planning-source-first-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    second = Path.join(System.tmp_dir!(), "planning-source-second-#{System.pid()}-#{System.unique_integer([:positive])}.json")
  ...
  test "pins active build orders before completed entries sorted by completion date" do
    directory = Path.join(System.tmp_dir!(), "planning-source-catalog-sort-#{System.pid()}-#{System.unique_integer([:positive])}")
