...
defmodule Aiur.Docs.ControlCenterFixture do
  ...
  defp configure_endpoint do
    ...
    config =
      ...
      |> Keyword.merge(
        ...
        open_tickets_fun: &__MODULE__.open_tickets/0,
  ...
  @doc false
  # The Tickets panel lists the repository backlog. This is the synthetic
  # equivalent of `Aiur.OpenTicketSource.snapshot/0`, which reads GitHub.
  def open_tickets do
    observed_at = DateTime.utc_now()

    tickets =
      Enum.map(open_ticket_plan(), fn {identifier, title, labels} ->
        %{
          identity: unit_identity(identifier),
          identifier: identifier,
          title: title,
          body_excerpt: "Example-only ticket body used for the documentation fixture.",
          url: "https://example.test/tickets/#{identifier}",
          state: "open",
          labels: labels,
          assignee: nil,
          created_at: DateTime.add(observed_at, -86_400, :second),
          updated_at: DateTime.add(observed_at, -1_800, :second)
        }
      end)

    %Aiur.OpenTicketSource.Snapshot{
      status: :available,
      generation: 1,
      observed_at: observed_at,
      truncated?: false,
  ...
  defp open_ticket_plan do
    [
      {"EX-154", "Add the example release health check", ["agent:todo", "complexity:2", "build-lane:core"]},
      {"EX-155", "Publish the example API reference", ["agent:todo", "complexity:2", "build-lane:api"]},
      {"EX-156", "Archive the example migration scripts", ["agent:todo", "complexity:1", "build-lane:data"]},
      {"EX-157", "Add per-workspace example usage alerts", ["agent:todo", "complexity:3", "build-lane:core"]},
      {"EX-158", "Harden the example deployment pipeline", ["complexity:3", "build-lane:platform"]},
      {"EX-159", "Add example load and soak coverage", ["complexity:3", "build-lane:quality"]},
      {"EX-160", "Localise the example application shell", ["complexity:3", "build-lane:web"]}
    ]
  end
