defmodule AiurSaturationReproTest do
  ...
  defp stub_env(status_rc, epmd_names) do
    dir = Path.join(System.tmp_dir!(), "aiur-repro-test-#{System.pid()}-#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
    on_exit_dir = dir
  ...
  defp watch(stub_daemon_state, dump_setup \\ "") do
    dir = Path.join(System.tmp_dir!(), "aiur-repro-watch-#{System.pid()}-#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
