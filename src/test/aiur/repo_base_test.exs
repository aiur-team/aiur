defmodule Aiur.RepoBaseTest do
  ...
  setup do
    tmp = Path.join(System.tmp_dir!(), "aiur_rb_#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  describe "base_branch/0" do
    ...
    setup do
      tmp = Path.join(System.tmp_dir!(), "rb_bb_#{System.pid()}-#{System.unique_integer([:positive])}")
      File.mkdir_p!(tmp)
  ...
  describe "server state machine" do
    setup do
      ...
      tmp = Path.join(System.tmp_dir!(), "rb_cfg_#{System.pid()}-#{System.unique_integer([:positive])}")
      File.mkdir_p!(tmp)
      cfg = Path.join(tmp, "config")
      File.write!(cfg, "tracker:\n  kind: memory\n")
      prev_path = Application.get_env(:aiur, :workflow_file_path)
