defmodule Aiur.Workspace.ProvisionerTest do
  ...
  test "local workspaces get command guards and a private scratch directory" do
    workspace = Path.join(System.tmp_dir!(), "aiur-provision-scratch-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "bulk workspace creation caps external POSIX after_create child builds" do
    test_root = Path.join(System.tmp_dir!(), "workspace-build-gate-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "ensure_workspace/2 on a valid existing checkout returns false and preserves contents" do
    tmp = Path.join(System.tmp_dir!(), "prov_#{System.pid()}-#{System.unique_integer([:positive])}")
    init_repo!(tmp)
  ...
  test "reports interrupted Git initialization as existing but refuses it for use" do
    workspace = Path.join(System.tmp_dir!(), "prov_unborn_#{System.pid()}-#{System.unique_integer([:positive])}")
    File.mkdir_p!(workspace)
  ...
  test "refuses unproven non-Git contents without deleting them" do
    workspace = Path.join(System.tmp_dir!(), "prov_unproven_#{System.pid()}-#{System.unique_integer([:positive])}")
    notes = Path.join(workspace, "notes.txt")
    File.mkdir_p!(workspace)
  ...
  test "completion proof makes an intentionally non-Git workspace reusable" do
    workspace = Path.join(System.tmp_dir!(), "prov_ready_#{System.pid()}-#{System.unique_integer([:positive])}")
    notes = Path.join(workspace, "notes.txt")
    File.mkdir_p!(workspace)
  ...
  test "partial agent-skill installation is not a non-Git completion proof" do
    workspace = Path.join(System.tmp_dir!(), "prov_partial_skills_#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "refuses an after_create hook that leaves a logs-only workspace non-Git, then self-heals on retry" do
    test_root = Path.join(System.tmp_dir!(), "prov_completion_#{System.pid()}-#{System.unique_integer([:positive])}")
    workspace_root = Path.join(test_root, "workspaces")
  ...
  test "ensure_workspace/5 marks a logs-only directory for initial provisioning" do
    workspace = Path.join(System.tmp_dir!(), "prov_logs_only_#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "remote setup refuses a logs-only workspace before a provider can receive its cwd" do
    test_root = Path.join(System.tmp_dir!(), "prov_remote_logs_#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "refuses a logs-only workspace whose logs node is not a safe directory" do
    workspace = Path.join(System.tmp_dir!(), "prov_invalid_logs_#{System.pid()}-#{System.unique_integer([:positive])}")
    File.mkdir_p!(workspace)
  ...
  test "ensure_workspace/2 on a stale plain file replaces it and returns created? true" do
    test_root = Path.join(System.tmp_dir!(), "prov_stale_#{System.pid()}-#{System.unique_integer([:positive])}")
    workspace_root = Path.join(test_root, "workspaces")
  ...
  test "resolve_branch_name/3 resumes the checked-out branch after a title edit" do
    root = Path.join(System.tmp_dir!(), "provisioner-branch-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "resolve_branch_name/3 resumes an open PR head after a deleted workspace" do
    workspace = Path.join(System.tmp_dir!(), "missing-#{System.pid()}-#{System.unique_integer([:positive])}")
    context = ticket_context("aiur/123-fix-login-and-signup")
