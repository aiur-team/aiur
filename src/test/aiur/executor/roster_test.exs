defmodule Aiur.Executor.RosterTest do
  ...
  setup do
    root = Path.join(System.tmp_dir!(), "aiur-executor-roster-#{System.pid()}-#{System.unique_integer([:positive])}")
    File.mkdir_p!(root)
  ...
  test "active requires this consumer's own acknowledgement, not a cursor that moved" do
    ...
    now = DateTime.utc_now()
    path = Path.join(System.tmp_dir!(), "aiur-roster-evidence-#{System.pid()}-#{System.unique_integer([:positive])}.json")
