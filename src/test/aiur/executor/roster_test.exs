defmodule Aiur.Executor.RosterTest do
  ...
  test "a stalled owner is reported stalled, not active", %{path: path} do
    ...
    roster =
      Roster.build(path: path, now: later_still, cursor: 5, pending_count: 42, stall_after_ms: @stall_after_ms)
    ...
    assert entry.cursor_moved == false
  ...
  test "active requires positive evidence that the cursor moved", %{path: path} do
    now = DateTime.utc_now()
    {:ok, _entry} = Claims.claim("agent-a", path: path, now: now)

    # Baseline observation at cursor 5, then the cursor advances.
    _first = Roster.build(path: path, now: now, cursor: 5, pending_count: 3, stall_after_ms: @stall_after_ms)
  ...
  test "two healthy executors are both listed and neither is flagged", %{path: path} do
    ...
    {:ok, _acked} = Claims.record_acknowledgement("agent-a", 5, path: path, now: now)
    {:ok, _acked} = Claims.record_acknowledgement("agent-b", 5, path: path, now: now)
    ...
    assert Enum.all?(roster.executors, &(&1.state == :active))
