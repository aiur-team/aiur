defmodule Aiur.GitHub.ResourceEventsTest do
  ...
  describe "the cached body is the change a viewer is waiting for" do
    ...
    # already recorded. `payload/1` is the only thing that decides whether a
    ...
    test "a first body publishes even though the validator did not move" do
      ...
      ResourceStore.put_payload(key, %{"body" => "hello"}, "W/\"same\"")

      assert_receive {:github_resource_changed, change}
      assert change.payload? == true
      assert ResourceStore.payload(key) == %{"body" => "hello"}
    end

    test "dropping a body publishes, because the page now has nothing to render" do
      key = key(15)
      ResourceStore.put_payload(key, %{"body" => "hello"}, nil)
      :ok = ResourceEvents.subscribe(key)

      ResourceStore.drop_payload(key)

      assert_receive {:github_resource_changed, %{payload?: false}}
    end
  ...
  describe "a write that changes nothing is silent" do
    ...
    test "re-writing an identical body publishes nothing" do
      key = key(18)
      ResourceStore.put_payload(key, %{"body" => "same"}, "W/\"e\"")
      :ok = ResourceEvents.subscribe(key)
    ...
    test "an edit to the same resource does publish" do
      key = key(19)
      ResourceStore.put_payload(key, %{"body" => "first"}, "W/\"e\"")
      :ok = ResourceEvents.subscribe(key)

      ResourceStore.put_payload(key, %{"body" => "edited"}, "W/\"e\"")

      assert_receive {:github_resource_changed, %{payload?: true}}
      assert ResourceStore.payload(key) == %{"body" => "edited"}
    end
  end
