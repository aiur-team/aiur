defmodule Aiur.GitHub.QuotaTest do
  ...
  test "attributes a named GraphQL operation when the request has no ticket" do
    quota = start_quota()

    Quota.observe(
      quota,
      graphql_request("query AiurCommentPollBatch { repository { id } rateLimit { cost } }", %{}),
      graphql_response(4974, 26)
    )

    assert [%{consumer: "github:AiurCommentPollBatch", cost: 26, estimated?: false}] =
  ...
  test "finds the agent-shell log through a dot directory under a tilde-form workspace root" do
    ...
    root = Path.join(System.tmp_dir!(), relative)
    ...
    tilde_root = Path.join("~", Path.relative_to(root, System.user_home!()))
