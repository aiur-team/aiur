defmodule Aiur.GitHub.ReadCacheTest do
  ...
  describe "policy coverage" do
    ...
    test "no default TTL outruns the tightest cadence a caller can be polling on" do
      ...
      #
      # `:repo_config` is the one class the short bucket does not pin to
      # 30 seconds: it rides the CIReadiness assessment cadence (its assessment
      # is itself cached for an hour) rather than the 30-second Build Order
      # window, so its short value is five minutes, not thirty.
      for class <- Policy.classes() -- [:repo_config] do
