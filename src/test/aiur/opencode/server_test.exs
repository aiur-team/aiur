defmodule Aiur.Opencode.ServerTest do
  ...
  test "ticket-directory prompt resolves the slot provider and persists" do
    ...
    try do
      ...
      # This goes through OpenCode's real SessionPrompt persistence path. A
      # legacy server selected through PATH attempts to write session_message
      # without its required seq and returns the production failure this guards.
      assert {:ok, _} = ApiClient.post_message(base_url, session["id"], %{parts: [%{type: "text", text: "operator regression prompt"}]})
    after
