defmodule Aiur.CodingAgentCheckpointTest do
  ...
  test "Codex adapter treats idle thread status as turn completion" do
    test_root = Path.join(System.tmp_dir!(), "aiur-codex-idle-status-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "Codex adapter ignores idle thread status before a turn starts" do
    test_root = Path.join(System.tmp_dir!(), "aiur-codex-prestart-idle-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  defp with_checkpoint_workspace(identifier, backend, script, body) do
    test_root = Path.join(System.tmp_dir!(), "aiur-checkpoint-#{identifier}-#{System.pid()}-#{System.unique_integer([:positive])}")
