defmodule Aiur.RunTelemetry.SummariesTest do
  ...
  setup do
    root = Path.join(System.tmp_dir!(), "aiur-summaries-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  defp with_tmp_reduce(fun) do
    dir = Path.join(System.tmp_dir!(), "aiur-reduce-#{System.pid()}-#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
    script = Path.join(dir, "reduce")
    File.write!(script, "#!/usr/bin/env bash\nprintf 'ok\\n'\n")
    File.chmod!(script, 0o755)
