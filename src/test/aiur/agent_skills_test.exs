defmodule Aiur.AgentSkillsTest do
  ...
  setup do
    tmp = Path.join(System.tmp_dir!(), "aiur_skills_#{System.pid()}-#{System.unique_integer([:positive])}")
    File.mkdir_p!(tmp)
  ...
  test "refuses skill roots redirected outside the workspace", %{workspace: ws} do
    external = Path.join(System.tmp_dir!(), "aiur_skills_external_#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "does not write exclusions through an external gitdir", %{workspace: ws} do
    git_dir = Path.join(System.tmp_dir!(), "aiur_skills_gitdir_#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "remote install does not write exclusions through an external gitdir", %{workspace: ws} do
    ...
    git_dir = Path.join(System.tmp_dir!(), "aiur_remote_skills_gitdir_#{System.pid()}-#{System.unique_integer([:positive])}")
    on_exit(fn -> File.rm_rf!(git_dir) end)
