defmodule Aiur.Orchestrator.GlobalPauseTest do
  ...
  describe "GlobalPauseStore recovery" do
    test "distinguishes a missing store from an unreadable store" do
      path = Path.join(System.tmp_dir!(), "global-pause-missing-#{System.pid()}-#{System.unique_integer([:positive])}.json")
    ...
    test "rejects a decoded store without a boolean pause flag" do
      path = Path.join(System.tmp_dir!(), "global-pause-invalid-#{System.pid()}-#{System.unique_integer([:positive])}.json")
      previous = Application.get_env(:aiur, :global_pause_store_path)
      Application.put_env(:aiur, :global_pause_store_path, path)
    ...
    test "keeps the store stable when the run log root changes" do
      state_dir = Path.join(System.tmp_dir!(), "global-pause-state-#{System.pid()}-#{System.unique_integer([:positive])}")
      run_one_root = Path.join(System.tmp_dir!(), "global-pause-run-one-#{System.pid()}-#{System.unique_integer([:positive])}")
      run_two_root = Path.join(System.tmp_dir!(), "global-pause-run-two-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  describe "idempotency and projection" do
    ...
    test "persists the global pause and provenance across orchestrator restart" do
      path = Path.join(System.tmp_dir!(), "global-pause-#{System.pid()}-#{System.unique_integer([:positive])}.json")
      previous = Application.get_env(:aiur, :global_pause_store_path)
      Application.put_env(:aiur, :global_pause_store_path, path)
    ...
    test "holds the fleet when persisted pause recovery is corrupt" do
      path = Path.join(System.tmp_dir!(), "global-pause-corrupt-#{System.pid()}-#{System.unique_integer([:positive])}.json")
