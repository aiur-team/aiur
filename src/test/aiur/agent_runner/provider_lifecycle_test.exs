defmodule Aiur.AgentRunner.ProviderLifecycleTest do
  ...
  test "cancelled pause retires the old provider turn before the resumed real turn" do
    pause_marker = Path.join(System.tmp_dir!(), "aiur-cancelled-pause-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "quota pause retires the failed provider turn before the resumed real turn" do
    pause_marker = Path.join(System.tmp_dir!(), "aiur-quota-pause-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "operator interrupt consumes the anonymous fallback before its queued turn" do
    ready = Path.join(System.tmp_dir!(), "aiur-operator-interrupt-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  defp prepare_case(name, script) do
    root = Path.join(System.tmp_dir!(), "aiur-provider-lifecycle-#{name}-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  defp lifecycle_barrier(name) do
    root = Path.join(System.tmp_dir!(), "aiur-#{name}-#{System.pid()}-#{System.unique_integer([:positive])}")
