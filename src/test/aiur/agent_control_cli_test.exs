defmodule Aiur.AgentControlCLITest do
  ...
  test "status reports active build-gate contention", %{orchestrator: pid} do
    gate_dir = Path.join(System.tmp_dir!(), "aiur-build-gate-status-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  test "status reports actionable legacy build-gate degradation" do
    gate_dir = Path.join(System.tmp_dir!(), "aiur-build-gate-legacy-#{System.pid()}-#{System.unique_integer([:positive])}")
    lock_dir = BuildGate.lock_dir(gate_dir)
    previous = Application.get_env(:aiur, :build_gate_dir_override)
  ...
  describe "prewarm status surface" do
    ...
    defp with_prewarm_enabled do
      tmp = Path.join(System.tmp_dir!(), "cli_prewarm_#{System.pid()}-#{System.unique_integer([:positive])}")
      File.mkdir_p!(tmp)
  ...
  describe "alerts/1" do
    test "alerts and watch use the default project ledger" do
      log_root = Path.join(System.tmp_dir!(), "aiur-default-alert-ledger-#{System.pid()}-#{System.unique_integer([:positive])}")
    ...
    test "prints persisted alerts as JSON lines with optional attention filtering" do
      workspace_root =
        Path.join(System.tmp_dir!(), "aiur-control-alerts-#{System.pid()}-#{System.unique_integer([:positive])}")
  ...
  describe "watch" do
    setup do
      ...
      root = Path.join(System.tmp_dir!(), "aiur-watch-#{System.pid()}-#{System.unique_integer([:positive])}")
      File.mkdir_p!(root)
    ...
    test "status and watch surface persisted open blocking operator asks", %{watch_root: root} do
      asks_root = Path.join(System.tmp_dir!(), "aiur-status-asks-#{System.pid()}-#{System.unique_integer([:positive])}")
