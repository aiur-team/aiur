defmodule Aiur.Opencode.OperatorText do
  @moduledoc """
  Pure opencode `<system-reminder>` Executor-wrapper wire shape.

  Extracts every wrapped Executor message from an opencode-formatted payload
  ...
  # Opencode wraps Executor-typed text in `<system-reminder>` envelopes before
  ...
  # so we forward only the Executor’s genuine text — exactly what Claude's
  ...
  # carry the Executor’s text. The extraction is kept because it stays correct
  ...
  # deleted the Executor-bearing block too, yielding "" — a silent drop the
  # agent could never answer (issue #332). Scanning every wrapper anywhere in
  # the text recovers the Executor’s message regardless of what surrounds it.
  ...
  @doc false
  # Pure derivation of the Executor-path trace fields, so the diagnostic
  # contract is unit-testable without Plug/app scaffolding. `wrapped` is true
  # only when a genuine, non-blank Executor message was buried in an opencode
  ...
  # Executor message was present but nothing was forwarded." Both fields and
  ...
  def normalize(text) when is_binary(text) do
    cond do
      msgs = scan_operator_messages(text) ->
        # One or more Executor messages buried in opencode wrappers (a folded
      ...
      String.contains?(text, "<system-reminder>") ->
        # No Executor-bearing wrapper, only opencode scaffolding — strip it.
  ...
  # Every Executor message buried in an opencode mid-stream wrapper, in order.
