defmodule Aiur.RunTelemetry.Sampler do
  ...
  def sample_once(previous, opts \\ []) when is_map(previous) and is_list(opts) do
    ...
    fd_headroom = safe_call(Keyword.get(opts, :fd_headroom_fun, &SystemFileDescriptors.sample/0), :unavailable)
    ...
    case safe_call(table_fun, {:error, {:procfs_unavailable, :reader_failed}}) do
      {:ok, table, table_warnings} when is_map(table) ->
        successful_sample(previous, table, table_warnings, entries, daemon_pid, operator_pid, fd_headroom, now_ms, clock_ticks, opts)

      {:error, reason} ->
        unavailable_sample(entries, daemon_pid, operator_pid, fd_headroom, reason)

      _other ->
        unavailable_sample(entries, daemon_pid, operator_pid, fd_headroom, :invalid_process_table)
    end
  end
  ...
  defp successful_sample(previous, table, table_warnings, entries, daemon_pid, operator_pid, fd_headroom, now_ms, clock_ticks, opts) do
    ...
    {measurements, measurement_warnings} =
      case safe_call(fn -> measure_fun.(table, pids) end, {:ok, %{}, [%{field: :procfs, reason: :measurement_failed}]}) do
    ...
    records = Enum.map(actors, &actor_record(&1, measurements, previous, now_ms, clock_ticks, fd_headroom))
  ...
  defp unavailable_sample(entries, daemon_pid, operator_pid, fd_headroom, reason) do
  ...
  defp ticket_specs(table, children, entries) do
    ...
    |> Enum.reduce({[], MapSet.new(), []}, fn {ticket, ticket_entries}, {specs, claimed, warnings} ->
      ...
      metadata = %{
        ...
        worker_hosts: ticket_entries |> Enum.map(& &1.worker_host) |> Enum.reject(&is_nil/1) |> Enum.uniq() |> Enum.sort()
      }

      spec =
        cond do
          remote? -> unavailable_spec("ticket:#{ticket}", "ticket", ticket, roots, :remote_worker, metadata)
          MapSet.size(pids) == 0 -> unavailable_spec("ticket:#{ticket}", "ticket", ticket, roots, :process_root_unavailable, metadata)
          true -> measured_spec("ticket:#{ticket}", "ticket", ticket, roots, pids, metadata)
        end

      warning = if not remote? and roots != [] and MapSet.size(pids) == 0, do: [%{event: :ticket_roots_unavailable, ticket: ticket}], else: []
  ...
  defp actor_record(%{availability: "unavailable"} = actor, _measurements, _previous, _now_ms, _clock_ticks, fd_headroom) do
  ...
  defp actor_record(actor, measurements, previous, now_ms, clock_ticks, fd_headroom) do
    ...
    if processes == [] do
      unavailable_record(%{actor | availability: "unavailable", unavailable_reason: "process_measurement_unavailable"}, fd_headroom)
    else
  ...
  defp partial_fields(processes) do
    Enum.filter(@metric_fields, fn field -> Enum.count(processes, &is_number(Map.get(&1, field))) < length(processes) end)
  end
    ...
    rates =
      Enum.flat_map(processes, fn process ->
        ...
        with %{observed_ms: observed_ms} = prior <- Map.get(previous, key),
          ...
          scale = if field == :cpu_ticks, do: 100.0, else: 1.0
          [delta / divisor / (elapsed_ms / 1_000) * scale]
        else
          _other -> []
        end
      end)
  ...
  defp tree_for_roots(table, children, roots) when is_map(table) do
    roots = Enum.filter(roots, &Map.has_key?(table, &1))
    expand_tree(roots, children, MapSet.new())
  end
  ...
  defp expand_tree([pid | rest], children, seen) do
    if MapSet.member?(seen, pid) do
      ...
      expand_tree(Map.get(children, pid, []) ++ rest, children, MapSet.put(seen, pid))
    end
  end
  ...
  defp safe_sample(sample_fun, previous) do
    case sample_fun.(previous) do
      ...
      _other ->
        %{records: [], warnings: [%{event: :resource_sample_failed, reason: :invalid_result}], previous: %{}}
    end
  rescue
    error -> %{records: [], warnings: [%{event: :resource_sample_failed, reason: Exception.message(error)}], previous: %{}}
  catch
    kind, reason -> %{records: [], warnings: [%{event: :resource_sample_failed, reason: {kind, reason}}], previous: %{}}
  end
