defmodule Aiur.GitHub.Budget do
  ...
  def request_resource(%{url: url}) when is_binary(url) do
    case URI.parse(url).path do
      ...
      # core pool. Billing a search request to `core` here — and, worse,
      # attributing a search-pool exhaustion response to a `core` hold below —
      # is exactly how a search rate limit became a fleet-wide `core` hold that
      # denied dispatch authorization (#2409).
      "/search/" <> _path -> "search"
      _path -> "core"
    end
  end
