defmodule Aiur.GitHub.ResourceEvents do
  ...
  `etag`, `version`, `source`, or `processed_at_ms` publishes.

  The body matters most and is easy to miss: `Aiur.GitHub.ResourceStore.payload/1`
  ...
  def publish({type, owner, repo, id} = key, entry) when is_map(entry) do
    change = %{
      ...
      # to stop.
      payload?: not is_nil(Map.get(entry, :payload)),
