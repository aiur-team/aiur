defmodule Aiur.GitHub.ResourceStore do
  ...
  @resource_types [
    ...
    # A single issue, as `GET /repos/{owner}/{repo}/issues/{number}` returns it.
    #
    # Three readers were each fetching this exact URL on their own schedule into
    # their own cache: the orchestrator's per-issue reconciliation poll, the
    # Build Order ticket-detail pane, and the dashboard's ticket panel. Keying on
    # the issue rather than on the reader collapses them to one read, which is
    # the whole reason this store is addressed by resource identity.
    #
    # It must be listed here: `@resource_types` is a closed set and `encode_key/1`
    # drops an unknown type at checkpoint with no error, so an entry keyed with a
    # type missing from this list would simply vanish on restart.
    :issue
  ]
