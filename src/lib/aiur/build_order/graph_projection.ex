defmodule Aiur.BuildOrder.GraphProjection do
  ...
  # A non-zero base for the "bound" roles a cadence still feeds when the
  # catalog is on-demand: a failed catalog read's retry delay, and the window
  # after which the catalog snapshot is shown as ageing. On-demand means *no
  # timer*, not zero-width backoff/staleness, so the bound falls back to the
  # tracker's base poll interval (mirroring `Aiur.BuildOrder.Cadence`'s own
  # fallback) when the cadence is `0`; a real cadence is its own bound. The
  # catalog's own read keeps this cadence-shaped bound (its boot/degraded fills
  # and their retries still ride the planning cadence), while a *selected*
  # root's bound is delivery latency instead (#2313) — the event-sourced
  # catalog no longer polls on a clock, so the planning cadence is not the
  # honest bound on how stale a watched root can be.
  ...
  # cadence (or its on-demand fallback); a selected root's retry is re-based on
  # delivery latency (#2313), because the event-sourced catalog no longer polls
  # on a clock — delivery latency is what bounds how stale the root can be
  # before the reconciliation re-reads it.
