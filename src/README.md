...
- `agent.target_load_average` enables the adaptive dispatch envelope (default `1.0`
  ...
  cap. Later capacity grows by `agent.load_ramp_step` below the target, and high
  samples halve it no more often than `agent.load_cooldown_seconds`. Set the target
  to `null` to use only the static cap and hard gate.
