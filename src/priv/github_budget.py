...
The broker deliberately persists only a SHA-256 credential fingerprint.  A
...
def acquire(args):
    ...
    try:
        ...
        hold_until = max(cooldown, resource_hold[0] if resource_hold else 0)
        ...
        if total >= args.max_inflight or family >= args.max_inflight_per_endpoint:
            conn.execute("COMMIT")
            print("wait 25")
            return

        if minute >= args.requests_per_minute:
        ...
        stagger = secrets.randbelow(args.stagger_ms) + 1 if args.stagger_ms > 0 else 0
