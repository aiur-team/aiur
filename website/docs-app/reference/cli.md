...
`aiur build-orders --json` uses schema version 2. Its completion objects report `progress_resolution: "empty"` with `progress: null` when a Build Order has no members, distinguishing an observed empty plan from 0% progress. Member, wave, lane, and graph completion objects on a selected root also carry `progress_stale_count` and `progress_stale_observed_at`: how many resolved members contribute their last known activity reading (an agent that paused keeps the percent it last reported) rather than a live reading or an accepted lifecycle completion, and the oldest such reading. Human output prints the same as `80% (last known 12m ago)` measured from `snapshot.captured_at`. An open member with no usable activity reading is `unresolved` rather than a resolved 0%, and a member closed without completing (not planned, duplicate, cancelled) is a resolved 0% regardless of any earlier reading. Catalog root completion is lifecycle-derived and does not carry these fields.
