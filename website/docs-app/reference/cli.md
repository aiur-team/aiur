...
| `aiur resume 142` | Resumes a paused ticket, including one whose pause is still pending, or starts an idle eligible ticket. It reports `already running` only when a live worker is working the ticket. An idle ticket held by an open blocking decision is refused with that decision's id; answer the decision, then resume. | `aiur resume 142` |
