...
| `scripts/aiurdev --clear` | Deletes every entry under `~/.aiur/logs/` before launch. It requires `--debug`; either test harness enables both automatically. In the agent IR sandbox it clears that sandbox's log parent instead, so do not combine it with a session whose logs must be retained. | `scripts/aiurdev --debug --clear` |
