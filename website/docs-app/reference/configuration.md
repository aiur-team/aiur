...
| `tracker.github.credentials` | array | `[]` | Additional GitHub credentials the daemon may spread API load across. Empty — the default — means one credential, resolved exactly as before. Each entry takes `id` (lowercase identifier), `kind` (`app_installation`, `machine_user` or `human`), `identity` (the GitHub login, for reports), `token_env` (env var holding the token; not needed for `app_installation`), `writes` (default `false`) and `enabled` (default `true`). The daemon picks per request the eligible credential with the most remaining budget for that resource, choosing GraphQL and Core independently. See [Credential pooling](/apis/github#credential-pooling). |
| `tracker.github.credentials[].writes` | boolean | `false` | Whether this credential may carry writes (comments, labels, merges, PR creation). A `human` credential can never be set to `true`: GitHub attributes the write to that person and it breaks the agent-authors / human-reviews separation Aiur's merge policy depends on. Leave writes on the identity that should own them and let extra credentials absorb read traffic, which is where the budget actually goes. |
