...
| `agent.build_gate_stall_timeout_ms` | integer | 300000 | Build-gate slot-occupancy watchdog. A slot held longer than this (ms) raises a needs-attention alert naming the command and holder; 0 disables it. Independent of `agent.stall_timeout_ms`, which only bounds turn silence (a long serial `--trace` run is never silent). |
