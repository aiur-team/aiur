...
- **Layering.** Shell exports win, then `./.env`, then `~/.aiur/.env`: the launcher reads the repository file first and each file fills only unset names, so the home file only fills the gaps. GitHub credentials (`GITHUB_TOKEN` and the `GITHUB_APP_*` set) resolve as one group: when `./.env` declares any of them, every GitHub credential in `~/.aiur/.env` is ignored, so a machine-wide App never outranks the token a repository configured for itself. A blank line such as `GITHUB_APP_ID=` counts as a declaration and is the way to opt a repository out of the global credentials. When the home value loses either way, the daemon logs a startup warning naming the variable (never its value).
