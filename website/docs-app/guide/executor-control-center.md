...
| --- | --- | --- |
| **Units** | `/`, the live fleet and its filters. | `aiur units` |
| **Commands** | `/decisions`, the durable decision inbox and each decision's detail. | `aiur commands` |
| **Build Order** | `/build-orders`, the Build Order catalog and one root's execution detail. | `aiur build-orders` |
| **Analytics** | `/analytics`, live-run telemetry and an optional Build Order scope. | `aiur analytics` |
| **Streamdeck+** | `/streamdeck`, browser emulator for the same live projection used by the authenticated physical Stream Deck + sidecar; [#1358](https://github.com/aiur-team/aiur/issues/1358) defines the remaining terminal hardware proof. | none |
