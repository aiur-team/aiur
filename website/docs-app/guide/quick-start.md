...
Run `aiur init` in the repository you want Aiur to work on. It scaffolds `.aiur/config`, `.aiur/hooks`, and `.aiur/prompt.md`, and writes `./.env` for tokens. Use `aiur init --force` to overwrite an existing scaffold.

The wizard asks for:

- tracker and repository settings;
- agent backends, routing, and limits;
- the GitHub token and lifecycle labels.
...
| `aiur watch` | Show a live board of tickets, state, and what each agent is doing. |
