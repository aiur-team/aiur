...
cache serves are predominantly GraphQL — `gh pr view`, `gh pr list`, `gh issue
view`, `gh issue list` all hit GitHub's GraphQL endpoint — and that endpoint
returns no `ETag` and no `Last-Modified`, so there is no validator to send and
no `304` to receive. The REST reads the wrapper does serve run inside the `gh`
binary, which does not expose request-header injection for subcommand reads.
