...
3. `agent:human-review` — the PR is ready for human review.
4. `agent:rework` — reviewer feedback requires another run, looping back through review.
5. `agent:merging` — the accepted PR is being merged.
6. `agent:done` — the work is complete.
