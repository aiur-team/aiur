...
The user-visible win is a much better chat experience (proper scrollback, themes, undo, attention notifications, search, multi-line editing) for free. The architectural win is that the pane stops being a custom UI we maintain.
...
- `elixir/lib/aiur/agent_pubsub.ex` — currently exposes `broadcast_transcript/2`, `broadcast_alert/2`, `broadcast_running_change/1`, `broadcast_status_change/2`, `subscribe_agent/1`. There is **no** generic `broadcast/2`. v3 adds `broadcast_turn_completed(identifier, payload)` and folds turn-completion delivery into the existing per-identifier topic that `subscribe_agent/1` already returns (per FEAS-03).
...
- **Turn-end signal: a new `Aiur.AgentPubSub.broadcast_turn_completed/2` is emitted from `Aiur.AgentRunner.codex_message_handler/4`** whenever the underlying CodingAgent's `:turn_completed` event arrives. The chat-completions SSE handler subscribes to the per-identifier topic via existing `Aiur.AgentPubSub.subscribe_agent/1` and pattern-matches `{:turn_completed, identifier, _payload}` to close the stream. opencode's `session.idle` event is ignored — it cannot serve this role (R13). The same mechanism handles `:turn_failed` and `:turn_cancelled` to close the stream on the failure path. (origin: FEAS-01)
...
- **Where does the turn-end signal come from?** → `:turn_completed` (also `:turn_failed`, `:turn_cancelled`) emitted by the existing `CodingAgent` backend. `Aiur.AgentRunner` rebroadcasts to PubSub via a new `broadcast_turn_completed/2`. opencode's `session.idle` is *not* used (would be circular per R13). (origin: FEAS-01)
- **Does `Aiur.AgentPubSub` already have a generic broadcast?** → No. v3 adds `broadcast_turn_completed/2` as a named helper. No generic `broadcast/2` is introduced. (origin: FEAS-03)
...
sequenceDiagram
    ...
    AC->>PB: broadcast_turn_completed(identifier)
    PB-->>BR: {:turn_completed, identifier, _}
...
- `opencode_json/1` accepts `%{bridge_url:, bridge_token:, identifier:, aiur_pid:}` and returns the rendered `opencode.json` map. All interpolation happens inside the function — callers pass values, not strings. The resulting map is `Jason.encode!`-ed by the workspace bootstrap step. The map declares exactly one `provider` (the Aiur bridge) and exactly one `model` per ADV-R2-10. An `aiur_metadata` key carries the spawning Aiur PID (used by U9's orphan-reap path per ADV-R2-02); opencode ignores unknown top-level keys.
...
- `chat_completion_chunk(content, opts)` returns the OpenAI chat-completion-chunk shape that opencode's `@ai-sdk/openai-compatible` adapter expects. The shape is documented in opencode's provider docs and the Vercel AI SDK source — any deviation gets a comment with a link.
...
**Goal:** Stand up the dedicated Bandit listener that exposes `POST /v1/chat/completions` to opencode. This is the *only* HTTP surface opencode needs from Aiur, and it's the inverse of what opencode normally talks to — instead of relaying to an LLM, the handler relays to `AgentChat.send/3` and streams back PubSub events as OpenAI chat-completion chunks. This unit *also* introduces the new `Aiur.AgentPubSub.broadcast_turn_completed/2` helper and wires `Aiur.AgentRunner.codex_message_handler/4` to emit it — the SSE handler is the consumer, and shipping the producer in the same unit keeps the change atomic.

**Requirements:** R2, R5, R9, R10, R11, R13

**Dependencies:** U1, U2.

**Files:**
- Create: `elixir/lib/aiur/opencode/bridge.ex` (Plug.Router with `/v1/chat/completions`, `/v1/health`)
- Create: `elixir/lib/aiur/opencode/bridge_supervisor.ex` (small Supervisor that wraps the Bandit listener with `:rest_for_one`, `max_restarts: 1`, `max_seconds: 5`, so a bind failure can't take down the rest of `Aiur.Supervisor` — ADV-R2-08)
- Create: `elixir/lib/aiur/opencode/chat_completions.ex` (the streaming handler — separate from the router for testability)
- Modify: `elixir/lib/aiur.ex` (add `Aiur.Opencode.BridgeSupervisor` as a child of `Aiur.Supervisor`, after `Aiur.HttpServer`)
- Modify: `elixir/lib/aiur/agent_pubsub.ex` (add `broadcast_turn_completed/2` reusing `AgentEvents.agent_topic/1` so existing `subscribe_agent/1` callers receive it — FEAS-03)
- Modify: `elixir/lib/aiur/agent_runner.ex` (extend `codex_message_handler/4` to detect `:turn_completed`, `:turn_failed`, `:turn_cancelled` events post-normalization and broadcast via the new helper — FEAS-01)
- Modify: `elixir/lib/aiur/opencode/config.ex` or new module: store the per-startup UUID shared secret in `:persistent_term` so the bridge handler and `WorkspaceSetup` (U8) both read the same value.
- Test: `elixir/test/aiur/opencode/bridge_test.exs`, `elixir/test/aiur/opencode/chat_completions_test.exs`, `elixir/test/aiur/agent_pubsub_test.exs` (turn-completion roundtrip), `elixir/test/aiur/agent_runner_test.exs` (turn-event broadcast)

**Approach:**
- **Bridge supervisor:** `Aiur.Opencode.BridgeSupervisor` is `:rest_for_one` with `max_restarts: 1` over `max_seconds: 5`. Its only child is the Bandit listener. If `Bandit` fails to bind, the supervisor's restart escalation hits its cap and the *supervisor* shuts down, but its parent `Aiur.Supervisor` (which uses a higher tolerance) absorbs that single subtree failure and the dashboard stays up. A `:warning` log at boot identifies the configured port and suggests overriding `opencode.bridge_port`. (origin: ADV-R2-08)
...
- **AgentPubSub turn-completion helper:** `def broadcast_turn_completed(identifier, payload \\ %{}) when is_binary(identifier)` broadcasts `{:turn_completed, identifier, payload}` on `AgentEvents.agent_topic(identifier)` — the same topic existing `subscribe_agent/1` callers already use. No new topic, no new subscription helper. (origin: FEAS-03)
- **AgentRunner wiring:** Inside `codex_message_handler/4` (after `normalize_event/1`), inspect the message's `:event` (or equivalent) field. When the value is `:turn_completed`, `:turn_failed`, or `:turn_cancelled`, call `AgentPubSub.broadcast_turn_completed(identifier, %{event: event, payload: payload})` *after* the existing `maybe_broadcast_transcript` call so transcripts always land before the close signal. Verify the Claude backend equivalent at `lib/aiur/claude/coding_agent.ex:373,499,593` produces a normalized message with the same shape; if not, normalize in this unit.
- **Router:** `use Plug.Router`, `plug :match`, `plug Plug.Parsers, parsers: [:json], json_decoder: Jason`, `plug :dispatch`. Bind via `Aiur.Opencode.Config.bridge_host/0` + `bridge_port/0`. Add `Plug.Logger` at `:debug` to keep dashboard logs cleaner.
- **Auth (SEC-001):** every request must carry `Authorization: Bearer <token>` matching the per-startup shared secret. Generated once on Aiur boot. Returned 401 otherwise.
...
- **Subscribe before send:** the handler calls `AgentPubSub.subscribe_agent(identifier)` *before* `AgentChat.send/3` so the first transcript event (or the turn-completed signal) cannot arrive before the subscription is in place.
- Routing to agent: `Aiur.AgentChat.send(identifier, sanitized_text, delivery_policy: :checkpoint, fallback: :queue_next)`.
- **R9 queued-pane auto-start with retry-race fix (ADV-R2-07):** on `{:error, :no_running_agent}` from `AgentChat.send`, the handler calls `Aiur.Orchestrator.resume_agent(identifier)`. On `{:ok, :started}` or `{:ok, :resumed}`, the handler waits up to 5 s for either a `{:status_change, identifier, :running}` message (already on the subscribed topic) or the first transcript chunk on the agent, then retries `AgentChat.send/3` **exactly once**. No further retries — repeated `:no_running_agent` after one resume cycle returns an error chunk. This bounds the retry loop and removes the race where a progress chunk arrives between `resume_agent/1` returning and the second `send/3`. On `{:error, :max_concurrent_agents_reached}` etc., return a stream-friendly error chunk.
- **Streaming response:** `Plug.Conn.put_resp_header("content-type", "text/event-stream") |> send_chunked(200)`, then a loop that pulls from the pre-established PubSub subscription on `AgentEvents.agent_topic(identifier)`. For each `{:transcript_event, %{role: :assistant, body: chunk}}` *or* `{:transcript_event, %{role: :command, body: summary}}`, emit a `chat.completion.chunk` shaped delta via `Aiur.Opencode.Protocol.chat_completion_chunk/2`. **`:user` transcript events are explicitly filtered out** — opencode renders the operator's input locally, so re-emitting would double-echo (FEAS-06). End the stream when `{:turn_completed, ^identifier, _}` arrives, or on `{:turn_failed, _}` / `{:turn_cancelled, _}` with an appropriate `finish_reason`. (origin: FEAS-01/R13)
...
- Integration: stream terminates after `AgentRunner` emits `:turn_completed` and `AgentPubSub.broadcast_turn_completed/2` lands on the subscribed topic; no further chunks emitted. Covers AE for R13/FEAS-01.
...
- Integration: `AgentPubSub.broadcast_turn_completed/2` is a no-op when there are no subscribers (doesn't raise). Covers FEAS-03.
...
- Modify: `elixir/lib/aiur/agent_runner.ex` (extract `transcript_event_from/1`; the new `:turn_completed` broadcast wiring lives in U4, not here)
...
- **Subscribe-then-read-with-dedup (FEAS-04).** The init sequence is: (1) `Aiur.AgentPubSub.subscribe_agent(identifier)` immediately, **before** opening the file; (2) start reading `logs/agent.ndjson`; (3) accumulate any `{:transcript_event, _}` messages that arrive in the GenServer mailbox while replay is in progress (they sit there harmlessly until `handle_info` runs); (4) after replay completes, drain the mailbox, dropping any event whose `timestamp` is `<=` the highest timestamp seen during replay, posting the rest. This closes the gap between "finished reading file" and "started listening" where a live event could otherwise be lost. `Aiur.AgentEvents.transcript_event/3` always carries a monotonically-ordered `timestamp`, and the same value lands in `agent.ndjson`, so dedup is exact.
- **Phase 1 — Replay.** `File.stream!("<workspace>/logs/agent.ndjson", [], :line)` is walked synchronously. Each line: `Jason.decode/1` → `transcript_event_from/1` (skip on `nil`) → convert via `Aiur.Opencode.Protocol.{user_message_part, assistant_text_message, assistant_command_message, system_message_part, alert_message_part}` per R7 → `Aiur.Opencode.ApiClient.post_message/3`. Skip malformed lines with `Logger.debug` and continue. Missing file returns `{:ok, 0}` (queued issue, AE for R9). Track the highest event timestamp seen.
- **Mailbox drain (between replay and live).** Switch into a tight loop that selectively receives any queued `{:transcript_event, event}` messages and processes each through the same `Protocol`-based publish path, dropping any whose `timestamp <= highest_replayed_ts`. Once the mailbox is empty, the init reply is sent — the PaneSession waits for this before reporting ready.
- **Phase 2 — Live.** `handle_info({:transcript_event, event}, state)` converts via `Protocol` and posts. **`:user` events are dropped** (R4 / F3) because opencode already renders the operator's local input. `:alert` events both POST to the session (chat entry) and call `Aiur.Opencode.ApiClient.show_toast/4` (toast). All other tags route through `post_message` only. Any `{:turn_completed, _, _}` / `{:turn_failed, _, _}` / `{:turn_cancelled, _, _}` messages on the same topic are ignored here — they are the bridge handler's concern, not the relay's.
...
- Error path: 3 consecutive transient `post_message` failures → logged but process continues; the 5th → process crash.
...
**Approach:**
- `materialize(workspace_path, identifier, bridge_url, bridge_token, opts)` is the public entry point. `opts` carries the opencode-server OS PID once known (or `nil` on the first write before the Port is opened — U9 re-runs `materialize/5` with the PID after `Server.await_ready/1` so a subsequent restart can find it).
- Calls `Aiur.Opencode.Protocol.opencode_json/1` with `%{bridge_url:, bridge_token:, identifier:, aiur_pid:}` to get a fully-built Elixir map; serializes via `Jason.encode!/1`. **No EEx templates** — the resulting JSON file is `Jason`-built end-to-end, which is injection-safe by construction (SEC-002 closed).
...
**Test scenarios:**
- Happy path: `materialize(tmp_workspace, "MT-123", "http://127.0.0.1:4097", "uuid-123")` produces `opencode.json` matching a checked-in snapshot fixture exactly.
...
- `init/1` sequence:
  1. `Aiur.Opencode.WorkspaceSetup.materialize/4` (U8) with the per-startup shared secret.
  2. `Aiur.Opencode.Server.start_link/1` (U5) — start opencode serve, wait for `await_ready/1`.
  3. `Aiur.Opencode.ApiClient.create_session/2` (U3) — store `session_id`.
  4. `Aiur.Opencode.TranscriptRelay.start_link/1` (U6) — runs replay synchronously, then transitions to live.
  5. `Aiur.Opencode.EventConsumer.start_link/1` (U7) — opencode SSE → Aiur.
  6. Transition to `:ready`, reply to callers waiting on `start/2`.
- `terminate/2` stops the three children in reverse order: EventConsumer → TranscriptRelay → Server (closing the Port).
- **Orphan reaping on restart (ADV-R2-02).** If `PaneSession` crashes (or `Aiur` itself restarts), the opencode OS process spawned by `Aiur.Opencode.Server` could be left orphaned. The Port closes when the GenServer dies, which SIGTERMs the wrapping `bash` — but a slow or wedged opencode process could survive briefly. U9's `init/1` queries for any *previous* `opencode.json` in the workspace that recorded the prior PID (extra field added in U2's template) and, if found, sends SIGTERM via `:os.cmd("kill #{pid} 2>/dev/null || true")` before spawning a new server. Best-effort; logged at `:info`. The "lazy spawn / kill on close" model means orphan windows are short and uncorrelated, so this is belt-and-suspenders, not a critical path.
...
- **Orphan reap (ADV-R2-02):** drop a stale `opencode.json` with a fake PID into a workspace, then `start/2` — assert that the fake PID was sent a TERM (via a recording wrapper around `:os.cmd`) before the new server spawned.
...
- New flow in `open_conversation/3`: (1) resolve workspace path for identifier; (2) `Aiur.Opencode.PaneSession.start(identifier, workspace_path)`; (3) tmux pane is respawned with `Aiur.Opencode.Protocol.attach_command(attach_url, session_id)` (uses Protocol so the CLI flag string lives in one place).
...
- Edge case: opencode startup fails → tmux pane is not respawned; operator sees an error toast/banner (assert via `AgentPubSub.broadcast_status_change`).
...
- Modify: `elixir/AGENTS.md` (add a one-paragraph note: the pane runs opencode; the bridge is the integration boundary; future modifications to chat UX go through `Aiur.Opencode.Protocol`; opencode SQLite store at `~/.local/share/opencode/` contains a copy of transcripts — operators should be aware)
...
- **Changes (v3):**
  - `Aiur.AgentPubSub` gains `broadcast_turn_completed/2` — additive on the existing per-identifier topic (`AgentEvents.agent_topic/1`). Existing subscribers continue to receive only the messages they pattern-match on; the new tuple shape `{:turn_completed, id, payload}` is ignored by current consumers (`AgentList.App`, `IssueLog`, etc.) which already match on `:transcript_event` / `:alert_event` tuples specifically. Verify via grep that no existing consumer uses a catchall `handle_info({_, _, _}, ...)` clause that would accidentally swallow this.
  - `Aiur.AgentRunner.codex_message_handler/4` gains a turn-event branch that calls `AgentPubSub.broadcast_turn_completed/2`. Pure additive; no existing call sites change behavior.
...
| Vercel AI SDK chat-completion chunk shape varies (tool-call deltas in particular) | The chunk shape lives in `Protocol.chat_completion_chunk/2`. Implement against the documented shape; verify with a real opencode build during U4. If quirks appear, contain them in `Protocol` and document the deviation. |
...
| Long-lived SSE connections leak under load | Each chat-completion SSE handler unsubscribes from PubSub on exit; tests assert subscription count returns to baseline after stream end. Bandit handles half-open connections natively. The `transport_options: [max_connections: 50]` cap (SEC-004) bounds the total. |
...
| Turn-end signal could be circular (opencode session.idle holds open the very SSE response that keeps it busy) | Turn-end is sourced from `Aiur.Codex.CodingAgent` / `Aiur.Claude.CodingAgent`'s existing `:turn_completed` event, rebroadcast via `Aiur.AgentPubSub.broadcast_turn_completed/2`. The bridge handler pattern-matches `{:turn_completed, identifier, _}` on the per-identifier topic. opencode's `session.idle` is observed for logging only. (FEAS-01 / R13) |
| Missing PubSub broadcast surface for turn-end signal | `Aiur.AgentPubSub.broadcast_turn_completed/2` added in U4 alongside the consumer. Uses the existing `AgentEvents.agent_topic/1` topic — no new subscription helper. (FEAS-03) |
