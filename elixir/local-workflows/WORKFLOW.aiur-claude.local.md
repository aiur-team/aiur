...
opencode:
  ...
  model_prefix: aiur
claude:
  # Wraps claude-app-server (`~/github/claude-app-server`). Installed
  # locally as `aiur-claude` on PATH (Homebrew tap `sapsaldog/symphony`
  # also distributes a build). Speaks the same JSON-RPC 2.0 NDJSON
  # protocol as the codex app-server.
  command: aiur-claude
  # Activates only when `agent.kind: claude` (see above). Available
  # models come from `model/list` on the running server; defaults today
