...
This directory contains the current Elixir/OTP implementation of Aiur, based on
[`SPEC.md`](../SPEC.md) at the repository root.

> [!WARNING]
> Aiur Elixir is prototype software intended for evaluation only and is presented as-is.
> We recommend implementing your own hardened version based on `SPEC.md`.

## Screenshot

![Aiur Elixir screenshot](../.github/media/elixir-screenshot.png)
...
During app-server sessions, Aiur can expose adapter-specific tools to repo workflows. For
...
Aiur stops the active run for that item and cleans up matching workspaces.
...
4. Optionally copy repo-local workflow skills such as `commit`, `push`, `pull`, and `land`.
   - Linear workflows can also use the `linear` skill, which expects Aiur's `linear_graphql`
...
## Operating Aiur with `aiur`

`scripts/aiur` is a thin wrapper around `./bin/aiur` that adds named
...
a fresh clone — `AIUR_REPO_ROOT` and `AIUR_MISE_BIN` remain available as
...
aiur --bg [profile|all]    # background mode (systemd on Linux, nohup on macOS)
aiur stop [profile|all]    # stop foreground processes and any background service
aiur list                  # show configured profiles
aiur build                 # rebuild bin/aiur explicitly
aiur --host [...]          # bind to the host configured in WORKFLOW.md (e.g. Tailscale IP)
aiur <path-to-WORKFLOW.md> # ad-hoc workflow in the foreground
```

`aiur` rebuilds `bin/aiur` automatically when it is missing or older
than any source file under `elixir/lib/`, `mix.exs`, or `mix.lock`, so a
`git pull` is all you need before invoking `aiur` again.

By default `aiur` injects `--host 127.0.0.1` so the Phoenix dashboard is
...
When `aiur` launches a profile (foreground or background), it sources three
optional files in order — later files override earlier ones, and all of them
are skipped if not present:

1. `~/.config/aiur-dashboard.env` (set `AIUR_ENV_FILE` to override path)
2. `<repo>/.env`
3. `<repo>/.env.local`
...
Define profiles in `~/.config/aiur/aiur.profiles`. Each non-comment line
...
```text
ops|/Users/you/code/ops|WORKFLOW.ops.md|4102|/Users/you/logs/ops|aiur-ops
```

The built-in profiles always loaded are:

- `default` and `aiur` — `local-workflows/WORKFLOW.aiur.local.md`, service `aiur`. Running `aiur` with no args dispatches this profile.
- `actions` — `local-workflows/WORKFLOW.actions.local.md`, service `aiur-actions`. Use `agents actions` to foreground this one.
...
Running `aiur` (no args) foregrounds only the `default` profile. Background
services are an explicit opt-in via `agents --bg [profile|all]`; bare `aiur`
...
| | Linux | macOS |
|---|---|---|
| Background driver | `systemctl --user` against a `<service>.service` user unit you maintain | `nohup` + PID file at `~/.local/state/aiur/<service>.pid` |
...
macOS, write your own `launchd` plist and use `aiur` in the foreground or via
the plist's `ProgramArguments`.

## Configuration

Pass a custom workflow file path to `./bin/aiur` when starting the service:

```bash
./bin/aiur /path/to/custom/WORKFLOW.md
```

If no path is passed, Aiur defaults to `./WORKFLOW.md`.

Optional flags:

- `--logs-root` tells Aiur to write logs under a different directory (default: `./log`)
...
- When `codex.turn_sandbox_policy` is set explicitly, Aiur passes the map through to Codex
  unchanged. Compatibility then depends on the targeted Codex app-server version rather than local
  Aiur validation.
- `agent.max_turns` caps how many back-to-back backend turns Aiur will run in a single
  invocation when a turn completes normally but the issue is still in an active state. Default: `20`.
- If the Markdown body is blank, Aiur uses a default prompt template that includes the issue
...
- If `WORKFLOW.md` is missing or has invalid YAML at startup, Aiur does not boot.
- If a later reload fails, Aiur keeps running with the last known good workflow and logs the
...
Run the real external end-to-end test only when you want Aiur to create disposable Linear
...
- `AIUR_LIVE_LINEAR_TEAM_KEY` defaults to `SYME2E`
- `AIUR_LIVE_SSH_WORKER_HOSTS` uses those SSH hosts when set, as a comma-separated list

`make e2e` runs two live scenarios:
- one with a local worker
- one with SSH workers

If `AIUR_LIVE_SSH_WORKER_HOSTS` is unset, the SSH scenario uses `docker compose` to start two
disposable SSH workers on `localhost:<port>`. The live test generates a temporary SSH keypair,
mounts the host `~/.codex/auth.json` into each worker, verifies that Aiur can talk to them
over real SSH, then runs the same orchestration flow against those worker addresses. This keeps
the transport representative without depending on long-lived external machines.

Set `AIUR_LIVE_SSH_WORKER_HOSTS` if you want `make e2e` to target real SSH hosts instead.
...
Launch your configured implementation agent in your repo, give it the path or URL to the Aiur
