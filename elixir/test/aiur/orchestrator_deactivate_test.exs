defmodule Aiur.OrchestratorDeactivateTest do
  ...
  describe "boot revival from human-review label" do
    test "materializes synthetic :deactivated entries for human-review issues not already running" do
      ...
      human_review_issues = [
        %Issue{id: "issue-revive-1", identifier: "BR-1", state: "human-review"},
        %Issue{id: "issue-revive-2", identifier: "BR-2", state: "human-review"}
      ]

      state = Orchestrator.revive_human_review_issues_for_test(base_state, human_review_issues)

      # Both ids now have synthetic running entries.
      entry1 = Map.fetch!(state.running, "issue-revive-1")
      entry2 = Map.fetch!(state.running, "issue-revive-2")

      for entry <- [entry1, entry2] do
        assert is_nil(entry.pid)
        assert is_nil(entry.ref)
        assert is_nil(entry.started_at)
        assert get_in(entry, [:control, :status]) == :deactivated
      end
    end

    test "skips entries already in state.running (race with regular dispatch)" do
      existing_pid =
        spawn(fn ->
          receive do
            :stop -> :ok
      ...
      base_state = %Orchestrator.State{
        running: %{
          "issue-revive-existing" => %{
            pid: existing_pid,
            ref: nil,
            identifier: "BR-EXISTING",
            issue: %Issue{id: "issue-revive-existing", state: "in-progress", identifier: "BR-EXISTING"},
            started_at: DateTime.utc_now(),
            control: %{status: :working}
          }
        },
        claimed: MapSet.new(["issue-revive-existing"]),
        codex_totals: %{input_tokens: 0, output_tokens: 0, total_tokens: 0, seconds_running: 0},
        retry_attempts: %{},
        max_concurrent_agents: 6
      }

      # Revive pass observes the same id with a human-review label.
      revived_issues = [
        %Issue{
          id: "issue-revive-existing",
          identifier: "BR-EXISTING",
          state: "human-review"
        }
      ]

      state = Orchestrator.revive_human_review_issues_for_test(base_state, revived_issues)

      # Existing entry is NOT overwritten by a synthetic one.
      entry = Map.fetch!(state.running, "issue-revive-existing")
      assert entry.pid == existing_pid
      assert get_in(entry, [:control, :status]) == :working
    end

    test "empty issues list is a no-op" do
      base_state = %Orchestrator.State{
        running: %{},
        claimed: MapSet.new(),
        codex_totals: %{input_tokens: 0, output_tokens: 0, total_tokens: 0, seconds_running: 0},
      ...
      state = Orchestrator.revive_human_review_issues_for_test(base_state, [])
