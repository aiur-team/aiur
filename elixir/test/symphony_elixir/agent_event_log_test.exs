defmodule SymphonyElixir.AgentEventLogTest do
  use ExUnit.Case, async: true

  alias SymphonyElixir.AgentEventLog

  defp tmp_workspace do
    path = Path.join(System.tmp_dir!(), "symphony-eventlog-#{System.unique_integer([:positive])}")
    File.mkdir_p!(path)
  ...
  describe "write/3" do
    ...
    test "returns :ok and logs at debug when the filesystem write would crash" do
      ...
      # path isn't taken.
      blocker = Path.join(System.tmp_dir!(), "symphony-eventlog-blocker-#{System.unique_integer([:positive])}")
      File.touch!(blocker)
