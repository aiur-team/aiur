defmodule SymphonyElixir.AlertsBroadcastTest do
  use SymphonyElixir.TestSupport

  alias SymphonyElixir.{AgentPubSub, Alerts}

  test "emit_custom broadcasts an alert on the agent topic when an identifier is supplied" do
    workspace_root =
      Path.join(System.tmp_dir!(), "symphony-alerts-bc-#{System.unique_integer([:positive])}")
  ...
  test "alerts without an identifier do not broadcast on a per-agent topic" do
    workspace_root =
      Path.join(System.tmp_dir!(), "symphony-alerts-nobc-#{System.unique_integer([:positive])}")
