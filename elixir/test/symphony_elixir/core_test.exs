defmodule SymphonyElixir.CoreTest do
  ...
  test "agent runner interrupts an active turn and drains the operator message as the next turn" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-agent-runner-interrupt-operator-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      :sys.replace_state(orchestrator_pid, fn state ->
        {queue_store, item} =
          SymphonyElixir.AgentQueue.operator_message("MT-253", "stop and answer this", delivery_policy: :interrupt)
          |> then(&SymphonyElixir.AgentQueueStore.enqueue(state.queue_store, &1))
