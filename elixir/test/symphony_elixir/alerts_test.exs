defmodule SymphonyElixir.AlertsTest do
  ...
  describe "alerts.yaml loading" do
    ...
    test "yields {} when the yaml file is unreadable or malformed", %{tmp_dir: tmp_dir} do
      ...
      original = Application.get_env(:symphony_elixir, :alerts_file_path)
      ...
      on_exit(fn ->
        if original do
          Application.put_env(:symphony_elixir, :alerts_file_path, original)
        else
          Application.delete_env(:symphony_elixir, :alerts_file_path)
        end
      end)
    ...
    test "returns an empty map when the yaml `alerts:` value isn't a map", %{tmp_dir: tmp_dir} do
      ...
      original = Application.get_env(:symphony_elixir, :alerts_file_path)
      Application.put_env(:symphony_elixir, :alerts_file_path, yaml_path)
  ...
  describe "default_player/2" do
    test "no-ops for http and https URL sounds" do
      assert :ok = SymphonyElixir.Alerts.default_player("http://example.test/ring.wav")
      assert :ok = SymphonyElixir.Alerts.default_player("https://example.test/ring.wav")
    end

    test "returns :ok when afplay is not on the path" do
      # First `cond` branch — `find_executable_fn` returns nil.
      assert :ok =
               SymphonyElixir.Alerts.default_player(
    ...
    test "resolve_workspace_for returns nil when the workspace_root_fn crashes" do
      ...
      assert is_nil(SymphonyElixir.Alerts.resolve_workspace_for("MT-WSCRASH", crashing))
    end

    test "resolve_workspace_for returns the workspace path when it exists" do
      workspace_root = Path.join(System.tmp_dir!(), "symphony-rwfor-#{System.unique_integer([:positive])}")
      File.mkdir_p!(Path.join(workspace_root, "MT-WSEXIST"))
      on_exit(fn -> File.rm_rf!(workspace_root) end)

      assert Path.join(workspace_root, "MT-WSEXIST") ==
               SymphonyElixir.Alerts.resolve_workspace_for("MT-WSEXIST", fn -> workspace_root end)
    end
    ...
    test "spawns a playback task when afplay and the sound both exist", %{tmp_dir: tmp_dir} do
      ...
      assert {:ok, _pid} = SymphonyElixir.Alerts.default_player(sound_path, stub_afplay)
    end
  end
  ...
  describe "playback fallbacks" do
    test "swallows a player function that raises and still emits" do
      ...
      workspace_root =
        Path.join(System.tmp_dir!(), "symphony-alert-rescue-#{System.unique_integer([:positive])}")
    ...
    test "default player skips http(s) sounds without trying to invoke afplay" do
      ...
      # we pass `player: &SymphonyElixir.Alerts.default_player_for_test/1`
      # if exposed — when not exposed we ensure no crash either way.
      workspace_root =
        Path.join(System.tmp_dir!(), "symphony-alert-url-#{System.unique_integer([:positive])}")
