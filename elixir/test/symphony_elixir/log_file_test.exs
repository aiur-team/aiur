defmodule SymphonyElixir.LogFileTest do
  ...
  describe "configure_level/0" do
    setup do
      ...
      original_env = System.get_env("SYMPHONY_DEBUG")

      on_exit(fn ->
        Logger.configure(level: original)

        case original_env do
          nil -> System.delete_env("SYMPHONY_DEBUG")
          value -> System.put_env("SYMPHONY_DEBUG", value)
        end
      end)
    ...
    test "sets debug level when SYMPHONY_DEBUG=1" do
      Logger.configure(level: :info)
      System.put_env("SYMPHONY_DEBUG", "1")
    ...
    test "sets debug level for truthy aliases" do
      for value <- ["true", "YES", " true ", "Yes"] do
        Logger.configure(level: :info)
        System.put_env("SYMPHONY_DEBUG", value)
        assert :ok = LogFile.configure_level()
        assert Logger.level() == :debug, "expected :debug for #{inspect(value)}"
      end
    end

    test "leaves level unchanged when SYMPHONY_DEBUG is unset" do
      Logger.configure(level: :info)
      System.delete_env("SYMPHONY_DEBUG")
