defmodule SymphonyElixir.ApplicationTest do
  ...
  alias SymphonyElixir.Application, as: SymphonyApp
  ...
  test "stop/1 is a no-op returning :ok" do
    ...
    assert :ok = SymphonyApp.stop(:any_state)
  end

  describe "start_distribution/1" do
    test "logs at info when distribution starts successfully" do
      assert :ok = SymphonyApp.start_distribution(SuccessStubDistribution)
    end

    test "logs at debug when distribution refuses to start" do
      assert :ok = SymphonyApp.start_distribution(FailureStubDistribution)
    end
  end
end
