...
# `comm` validates input order with the ambient collation, so it must use the
# same C collation the two inputs were sorted under. Without this the order
# check fails and the guard silently under-reports untouched deletions.
