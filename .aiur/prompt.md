...
- For real implementation tickets, before writing code read `src/.formatter.exs` and Credo's project settings in `src/mix.exs` so changes are lint-clean on the first pass. Keep changes small, add tests, and run the scoped local pre-PR gate before opening/finalizing a PR: `mix compile --warnings-as-errors`, `mix format`, and affected tests only (the test files for modules you touched plus directly related tests). Run `mise exec -- mix aiur.affected_tests` to compute that scoped set deterministically — it prints the exact `mix test --max-cases 4 <files>` line to run, or advises `make ci` when the change (gettext/mix/config) can't be scoped safely. Run every affected-test invocation with `mix test --max-cases 4` so one agent cannot monopolize the host. Fix failures in this scoped gate, then push to `origin` and open a PR against the configured `tracker.base_branch` (`main` in this repository). Do not run Credo locally. Do not gate PR-opening on a clean full-suite `mix test` run or loop on unrelated suite flakes; CI runs the authoritative full lint and full test suite through `make ci` on every PR.
