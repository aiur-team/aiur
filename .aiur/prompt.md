...
- Once a PR has been ready for review, approved, or entered the merge queue, never convert it back to draft. This includes approval history GitHub reports as `DISMISSED`: push repair commits and leave the PR ready while CI and branch rules prevent merging. Automated agents must not use `gh pr ready --undo`, `convertPullRequestToDraft`, or review-dismissal APIs. If an exceptional draft transition is genuinely unavoidable, comment on the PR with the reason and ask the Executor to perform it; never make the transition silently.
