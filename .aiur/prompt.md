...
- Work in the current workspace checkout on its generated canonical branch. Set `workspace="$AIUR_AGENT_WORKSPACE"` and select repository context with `git -C "$workspace"`; never `cd` into a repository to run Git or rely on the ambient working directory. This rule overrides Git examples in every installed skill: translate ambient or `cd`-based snippets to `git -C "$workspace"`. Read the branch with `git -C "$workspace" branch --show-current`; never reconstruct a bare `aiur/{{ issue.identifier }}` ref. The `.git` directory IS writable here — do NOT copy it to `.git-writable` (that's a pattern from the GitHub Actions workflow, not this one). Do not set `GIT_DIR` or `GIT_WORK_TREE`.
- Before integrating the configured base into an existing feature branch, record the exact pre-integration head and create a rescue ref for it; push that rescue ref before resolving any nontrivial conflict. Resolve conflicts hunk-by-hunk while preserving both the feature contract and the new base intent. Never resolve a conflicted file wholesale with `--ours`, `--theirs`, `git -C "$workspace" checkout <base> -- <file>`, or by resetting the feature branch to the base. Before pushing the refreshed feature branch, compare `git -C "$workspace" diff --stat origin/$AIUR_BASE_BRANCH...HEAD` with the prior PR scope and prove the intended feature diff remains. If the diff disappears or shrinks unexpectedly, stop, restore from the rescue ref, and alert the Executor instead of pushing.
