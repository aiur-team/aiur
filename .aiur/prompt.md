...
- Work in the current workspace checkout on its generated canonical branch. Set `workspace="$AIUR_AGENT_WORKSPACE"` and select repository context with `git -C "$workspace"`; never `cd` into a repository to run Git or rely on the ambient working directory. Read the branch with `git -C "$workspace" branch --show-current`; never reconstruct a bare `aiur/{{ issue.identifier }}` ref. The `.git` directory IS writable here — do NOT copy it to `.git-writable` (that's a pattern from the GitHub Actions workflow, not this one). Do not set `GIT_DIR` or `GIT_WORK_TREE`.
