...
name: linear
description: |
  Use Aiur's `linear_graphql` client tool for raw Linear GraphQL
  operations such as comment editing and upload flows.
---

# Linear GraphQL

Use this skill for raw Linear GraphQL work during Aiur app-server sessions.

## Primary tool

Use the `linear_graphql` client tool exposed by Aiur's app-server session.
It reuses Aiur's configured Linear auth for the session.
