...
Aiur returns exact-arithmetic, scope-labelled token and dollar totals by run/build, ticket, agent family, backend, and resolved model using separately comparable provider-reported estimate or versioned API-equivalent estimate bases, with explicit retained coverage and unknowns.
...
- Apply the price revision effective at the observation's occurrence time and exact resolved model. Preserve cached input, cache-creation input, and output/reasoning pricing distinctions without double counting token subsets. Historical results remain stable when a new price revision is added.
- Expose a query over an explicit typed ticket set and/or run set returning tokens and monetary totals by basis, plus groups by ticket, agent family, backend, resolved model, auth mode, and run. Include unknown contributors, pricing/model coverage, ledger health, and earliest/latest retained coverage.
...
- For subscription auth, return API-equivalent estimate metadata requiring `*`, information-popover copy key, and actual-tier join requirement. It must not be named billed or actual spend. For mixed bases, return separate buckets; never produce a combined dollar total across unlike bases.
...
- Unlike bases and currencies are never arithmetically combined. Unknown contributors remain in token totals and make monetary coverage partial/unknown.
...
- Group sums reconcile exactly to their matching basis/scope total; unrelated identities cannot enter through bare-number collisions.

## Refreshable implementation notes

- Seed price fixtures from an explicitly reviewed revision at pickup; record source/effective dates but perform no runtime web fetch.
- Keep estimates reproducible by storing the revision table in source/config and selecting by event occurrence time. Add a coverage reason when an observation predates known pricing.
...
- Exact-arithmetic/property tests cover each basis, model/token price dimension, cache/reasoning subsets, price revision boundaries, unknown model/auth/price, mixed bases/currencies, completed and unrelated tickets, and group-to-total reconciliation.
...
- Reads: DASH-009 ledger/query behavior; versioned price table; explicit typed run/ticket membership sets.
- Writes: cost/usage grouping projection and query API, pricing revisions/fixtures, tests.
- Contracts: comparable cost bases, exact grouped summary, current-build retained-membership semantics.

## Sibling boundaries and open gates

DASH-010 supplies required Remote Control observations but does not block developing this projection against fixtures. DASH-012/013 own actual account tier and quota facts. DASH-015 must keep bases separate and apply the subscription estimate disclosure exactly.
