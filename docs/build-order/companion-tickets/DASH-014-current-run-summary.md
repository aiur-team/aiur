...
- Compute `live_count` from DASH-002 `Live`, and `remaining_count` as nonterminal current-run members in `Unfinished`. Report terminal successful, terminal non-work/cancelled, unknown-state, and total member counts separately.
...
Build a pure projection over DASH-002's catalog plus `Aiur.Boot` run identity/start metadata and existing progress provenance. Reuse injected clock, PubSub generation, and health patterns.

## Contract and invariants

- Counts call DASH-002 predicates; this projection cannot redefine Live, Unfinished, or terminal membership.
...
- Reads: DASH-002 current-run catalog, canonical run identity/start, progress/complexity provenance.
...
DASH-015 renders this summary. DASH-011 separately aggregates run/build usage and money. Build Order readiness/critical-path calculations remain outside this current-run estimator.
