...
Aiur exposes a redacted, versioned Codex provider-meter projection that truthfully represents subscription and API-key account modes, multiple quota/rate windows, plan tier, sparse updates, reset times, freshness, and health without treating scheduling fallback as financial truth.
...
- Define a versioned `ProviderMeterSnapshot` keyed by provider/backend and opaque account generation, with auth mode, plan/tier and source, update kind (`snapshot` or `patch`), observed/ingested times, source/version, health, and windows keyed by stable `limit_id`.
...
- Create a new account generation when the trusted Codex auth/session handshake changes. Never merge last-known-good windows across generations or accounts, and do not persist raw account identity to accomplish this.
...
- Use one provider's update to overwrite another provider or account generation.

## Existing owner and reuse target

Extend the Codex account/rate-limit protocol client/normalizer and add a dedicated provider-meter projection. Reuse protocol versioning, app-server lifecycle, trusted auth-mode detection, and health/PubSub patterns. Treat `Aiur.ModelAvailability` as a compatibility consumer only.

## Contract and invariants

- Snapshot, patch, tombstone, failure, stale, unsupported, and empty-supported are distinct states.
- Provider, backend, account generation, and limit ID are part of identity. Sparse updates cannot delete unrelated windows; full snapshots cannot leave removed windows alive.
...
- Increment opaque account generation from trusted process/auth lifecycle, not a persisted email/account ID.
...
- Fixture/property tests cover camelCase normalization, multiple/arbitrary IDs, full snapshot removal, sparse patch, explicit tombstone, account-generation change, subscription and API-key modes, reset/credit fields, per-window stale/expiry, failure/recovery, and provider isolation.
...
- Rebase on current Codex protocol work; run account/rate-limit fixtures, app-server lifecycle, availability compatibility, provider projection, redaction, packaging, and full CI suites.
...
- Reads: structured Codex account/rate-limit protocol and trusted auth/process generation.
- Writes: provider-meter schema/projection, Codex adapter, health/PubSub, availability compatibility, fixtures/tests.
- Contracts: snapshot/patch/tombstone semantics, account generation, per-window freshness, subscription/API-key truth.

## Sibling boundaries and open gates

DASH-013 implements Claude against this contract, and DASH-015 renders both. DASH-011 cost estimates may join plan tier later but does not own meter ingestion.
