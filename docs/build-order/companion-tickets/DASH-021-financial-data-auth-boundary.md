...
**Serializes with:** dashboard authentication, provider loaders, PubSub subscriptions, and protected API/cache changes
...
- Classify as protected all usage tokens/cost/groups and provider meter/account facts: provider/backend, auth mode, plan/tier, quota/rate/credit/spend controls, percentages/limits, reset times, freshness, health, and retained LKG values.
...
- Ensure protected values never enter denied LiveView assigns, rendered HTML, client events, serialized diffs, connection-scoped caches, logs, telemetry metadata, or generic observability/state APIs.
...
- Put protected values in a hidden DOM node or redact them only after provider query/subscription.
...
- Nonfinancial DASH-014 facts remain available according to existing dashboard policy and never carry protected accounting fields.
...
- Inventory every generic dashboard/observability endpoint, assign, cache, and PubSub topic that could expose the protected field classes before writing the policy.
...
- Nonleakage tests prove denied mode never invokes providers and contains none of the sentinel token, cost, group, account, auth-mode, plan/tier, quota/rate/credit, percentage/limit/reset, freshness, or LKG values in HTML, assigns, LiveView diffs/events, subscriptions, caches, logs, telemetry, or generic APIs.
...
- No protected value enters logs, prompts, bug reports, telemetry labels, unauthenticated caches, or generic APIs. Use synthetic fixtures in evidence.
...
- Contracts: server-side financial-data access and protected-delivery boundary.
