...
- Build Order: 16 executable/capstone tickets, 61 points
- Standalone dashboard companions: 22 tickets, 75 points
...
```mermaid
graph TD
  B1[BO-001 Domain] --> B2[BO-002 GitHub graph adapter]
  B2 --> B3[BO-003 Root catalog/graph LKG]
  B4[BO-004 Typed event identity]
  ...
  B9 --> B10
  B4 --> B16[BO-016 Generic ticket context]
  B8 --> B16
...
BO-001, BO-004, and BO-008 are the independent initial implementation nodes;
...
Build Order finishes only when BO-001 through BO-016 are implemented, reviewed,
...
The twenty-two dashboard companions, Linear parity #1067, skill-delivery tracking,
...
Materialize one non-dispatchable root plus sixteen direct native sub-issues.
...
family. Materialize the twenty-two companions separately with complexity and
`model:codex`, no Build Order parent/phase/lane, and their real native blockers.

Every one of the 40 bodies links the real immutable approval commit and carries
