...
| Dashboard and API | `AiurWeb.Presenter` projects only the live orchestrator snapshot (running and retrying). `AiurWeb.DashboardLive` refreshes through `AiurWeb.ObservabilityPubSub` and sends writes through `AgentChat`. Browser writes fail closed behind `dashboard_writable`; REST writes additionally pass the configured basic-auth policy, same-origin checks, and `X-Aiur-Request`. | Compose Decision and outcome projections into `Presenter`/`DashboardLive`; route LiveView and API mutations through the same public `DecisionStore` API, preserving the existing writable and request-security gates. |
...
Validation before append must bound identifier and text sizes, reject malformed
artifact paths/URLs and unsafe control data, and redact known credential forms.
Do not ingest environment dumps or raw agent transcripts. Treat stored Markdown
as data, not trusted HTML, and sanitize it again at every HTML or API rendering
boundary. OCC-1 owns the exact schema and limits, but these controls are part of
the persistence contract rather than optional UI hardening.
...
relational constraints that cannot be enforced in the domain service, or query
...
| `run_id` | One daemon boot | Canonical OCC run/window |
...
“Observed during this run” is an optional factual badge when `merged_at` falls
inside the run window; it must not be rendered as “merged by this run.” True run
attribution requires an explicit durable association made when Aiur opens or
adopts the PR (run ID + attempt ID + PR number), followed by matching the merge
event to that association. That association does not exist today.
...
across pending, delivered, consumed, restored, and failed states for the VM
lifetime, returning the previously accepted queue handle when `DecisionStore`
retries the same action after its own process restarts.
...
After a daemon restart the transient queue and its index are gone, so startup
reconciliation retries every durable intent without a terminal dispatch result;
the stable action ID still correlates the new queue item to the original intent.
OCC-3 may extend operator queue items and transition notifications, but must not
...
- **OCC-3:** implement the durable outbox/correlation contract over
  ...
  expose queue transitions and require explicit agent ack.
...
- **OCC-5:** treat `run_id` as daemon scope while showing all tracker work seen
