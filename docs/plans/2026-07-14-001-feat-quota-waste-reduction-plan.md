...
- **Revised lever:** cold reworks happen because resume doesn't fire, not because the worktree is gone. Resume is driven by `SessionResume`/`SessionHandle` (`agent_runner/session_lifecycle.ex`, `session_resume.ex`): `opts[:resumed]` is set only when a persisted `SessionHandle` exists for the identifier+backend and `CodingAgent.resumable?` holds.
- **Investigation result (2026-07-14, from logs + code):**
  - The `SessionHandle` is stored **host-global** (`Paths.log_root_dir()/<repo>.<id>.session.json`), so it survives the `before_run` workspace re-clone — the handle is NOT the loss point.
  - Resume, when attempted, **always succeeds**: logs show **166 "Resumed prior agent session", 0 "degraded to a clean start"**.
  - Yet most re-dispatches create fresh rollout threads without a resume log → `SessionResume.load_resume_thread_id/3` returns `nil` **before** loading the handle. Its gate is `worker_host == nil and CodingAgent.resumable?(backend)`, and `SessionHandle.validate/4` also requires matching schema/backend/**hostname**. Prime suspects: dispatches carrying a **non-nil `worker_host`** (resume skipped entirely) or a hostname mismatch (`:none`, logged only at DEBUG). Pinning this down is a **debugging spike** (enable debug logging / trace one rework dispatch), not a code change.
  - **Strategic consequence:** **U1 already captures the primary win.** Whenever a rework is dispatched cold — for ANY of the above reasons — it now gets the rework-continuation prompt instead of re-running brainstorm+plan. U2 is a *smaller incremental* optimization (true thread continuity + saved cold first-turn overhead) that is only worth doing after the debugging spike identifies why resume isn't attempted. **Deprioritized relative to U1's already-shipped mitigation.**
