...
- Closed source vocabularies: the 2.1.210 adapter accepts only the reviewed query-source and effort values (or their explicitly reviewed bounded identifier grammar) for that revision. Main-thread and compaction sources remain exact; bounded subagent names normalize to the content-free `subagent` category before publication. An authenticated producer still cannot smuggle arbitrary content through an allowlisted key.
