...
- **Headless `claude` OS-process orphan on brutal-kill teardown (found during U9 manual verification 2026-06-08; fixed + live-verified 2026-06-08):** `terminate_task/1` kills the BEAM AgentRunner task, but a headless backend configured with a multi-statement `command:` can leave a grandchild that reparents to init and survives. This affects every brutal-kill stop path (`terminate_running_issue`, `deactivate_running_issue`, and U9's `teardown_for_redispatch`), not U9 alone. Fix (shipped): track the headless bash os_pid on the running entry as `:headless_os_pid` (mirroring `:repl_os_pid`) and reap its subtree via `RemoteControl.graceful_kill_tree/1` (recursive `pgrep -P` descendant walk + per-pid graceful SIGTERM→SIGKILL) inside the shared `kill_repl_session`, under the R6 "never break headless fallback" guard. **Live finding:** the *production* `aiur-claude` binary is a single `exec node …`, so `bash -lc "<scrub>; aiur-claude"` exec-optimizes the final command — the tracked os_pid *becomes* node directly (no fork, zero descendants), and a single graceful kill already reaps it cleanly. The subtree reap is the correct superset: it is a no-op for the real binary and reaps the orphan only for orphan-producing multi-statement configs (covered by the `graceful_kill_tree/1` unit test). Verified live by spawning the real `aiur-claude` exactly as `CodingAgent.start_port/1` does, confirming the pid resolves to `node …/dist/index.js` with no descendants, then reaping via the real `graceful_kill_tree/1` to zero residual processes.
- **End-of-build dead-code pass (completed 2026-06-08):** the U9 label-driven `r`-key redesign orphaned the entire old `claude remote-control --spawn` GenServer path. Removed under the R6 guards (codex + headless fallback untouched): the `Aiur.Claude.RemoteControl` GenServer lifecycle (`start_link`/`init`/`handle_*`/`terminate`, `--spawn` `build_command`, `start_port`, `session_url`, `stop`, `build_handoff` + transcript-handoff helpers, `delete_debug_files`, `default_debug_file`, `scan_for_url`, `notify`, `shell_escape`) — the module is now a stateless utility (kept: `parse_session_url`, `ensure_workspace_trusted`, `workspace_slug`, `resolve_transcript_path`/`newest_transcript`/`read_transcript`, `graceful_kill`/`graceful_kill_tree`, `reap_orphaned_servers`); the orchestrator's four dead handlers (`{:remote_control_url|ready|exit}`, the RC-ref branch of `:DOWN`) plus `update_remote_control_url`/`mark_remote_control_ready`/`clear_remote_control_for_server`/`drop_remote_control`/`find_rc_entry`/`stop_running_remote_control`/`stop_remote_control_server`/`remote_control_active_entry?`/`delete_remote_control_handoff` and their live-path call sites (`terminate_running_issue`, `teardown_for_redispatch`, `reactivate_issue`) plus the dead `_for_test` exports; and the `Aiur.Claude.RemoteControl.Supervisor` DynamicSupervisor child. `remote_control_summary/1` now derives purely from the `model:claude-remote` label. Net −695/+30 lines across 4 files; full suite green (the 2 residual failures — `ShutdownCleanupTest` script-regex, `LsRemoteTicker` async-timing — are pre-existing and unrelated).
